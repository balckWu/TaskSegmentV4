...
def build_support_bank(
    ...
    for organ_name in support_dataset.organ_to_positive_indices.keys():
        ...
        for idx in select_support_indices(support_dataset, organ_name, max_support_per_domain, seed):
            ...
            # 🌟 EMSD 核心修改：强制提取唯一宏观锚点 [CLS] 作为检索依据，杜绝均值池化
            if text_tokens is not None and text_tokens.shape[1] > 0:
                text_descriptor = normalize_descriptor(text_tokens[:, 0, :]).detach().cpu()
            else:
                text_descriptor = fallback_text
...
def get_projected_text_descriptor(model, text_feats, device):
    ...
    if projected is None:
        return None
    # 🌟 EMSD 核心修改：查询图同样强制提取 [CLS] 进行匹配
    return normalize_descriptor(projected[:, 0, :])
